...
public:
  virtual int connect(const char *host, uint16_t port) {
    //Serial.println(F("##virtual int connect(const char *host, uint16_t port)"));
    TINY_GSM_YIELD();
    rx.clear();
  ...
  virtual int connect(IPAddress ip, uint16_t port) {
    //Serial.println(F("##virtual int connect(IPAddress ip, uint16_t port)"));
  ...
  virtual void stop() {
    //Serial.println(F("##virtual void stop()"));
    TINY_GSM_YIELD();
  ...
  virtual size_t write(const uint8_t *buf, size_t size) {
    //Serial.println(F("##virtual size_t write(const uint8_t *buf, size_t size)"));
    TINY_GSM_YIELD();
    at->maintain();
  ...
  virtual size_t write(uint8_t c) {
    //Serial.println(F("##virtual size_t write(uint8_t c)"));
    return write(&c, 1);
  }

  virtual int available() {
    //Serial.println(F("##virtual int available()"));
    TINY_GSM_YIELD();
...
*/
   ...
   */
   ...
   virtual int read(uint8_t *buf, size_t size) {
     //Serial.println(F("##virtual int read(uint8_t *buf, size_t size)"));
     TINY_GSM_YIELD();
     at->maintain();
     size_t cnt = 0;
     uint8_t i = 0;
     while (cnt < size) {
       ...
       if ( i > 10 ) {
         ...
         break;
     	  ...
     	  //dbg
     	  Serial.println(cnt); Serial.println(" < "); Serial.println(size);
        size_t chunk = TinyGsmMin(size-cnt, rx.size());
     	  //dbg
  ...
  virtual int read() {
    //Serial.println(F("##virtual int read()"));
    uint8_t c;
    if (read(&c, 1) == 1) {
      return c;
    }
  ...
  virtual uint8_t connected() {
    //Serial.println(F("##virtual uint8_t connected()"));
    if (available()) {
      return true;
    }
    return sock_connected;
...
public:
   ...
   */
  bool begin() {
    //Serial.println(F("##bool begin()"));
    return init();
  }

  bool init() {
    //Serial.println(F("##bool init()"));
    if (!testAT()) {
      return false;
    }
  ...
  void setBaud(unsigned long baud) {
    //Serial.println(F("##void setBaud(unsigned long baud)"));
    sendAT(GF("+IPR="), baud);
  }

  bool testAT(unsigned long timeout = 10000L) {
    //Serial.println(F("##bool testAT(unsigned long timeout = 10000L)"));
  ...
  void maintain() {
    //Serial.println(F("##void maintain()"));
  ...
  bool factoryDefault() {
    //Serial.println(F("##bool factoryDefault()"));
  ...
  String getModemInfo() {
    //Serial.println(F("##String getModemInfo()"));
    sendAT(GF("I"));
    String res;
  ...
  bool poweroff() {
    //Serial.println(F("##bool poweroff()"));
    sendAT(GF("+CPOWD=1"));
    return waitResponse(GF("NORMAL POWER DOWN")) == 1;
  }

  bool radioOff() {
    //Serial.println(F("##bool radioOff()"));
    sendAT(GF("+CFUN=0"));
    if (waitResponse(10000L) != 1) {
      return false;
    }
  ...
  */
  bool sleepEnable(bool enable = true) {
    //Serial.println(F("##bool sleepEnable(bool enable = true)"));
  ...
  SimStatus getSimStatus(unsigned long timeout = 10000L) {
    //Serial.println(F("##SimStatus getSimStatus(unsigned long timeout = 10000L)"));
    for (unsigned long start = millis(); millis() - start < timeout; ) {
      sendAT(GF("+CPIN?"));
  ...
  RegStatus getRegistrationStatus() {
    //Serial.println(F("##RegStatus getRegistrationStatus()"));
    sendAT(GF("+CREG?"));
  ...
  String getOperator() {
    //Serial.println(F("##String getOperator()"));
    sendAT(GF("+COPS?"));
  ...
  int getSignalQuality() {
    //Serial.println(F("##int getSignalQuality()"));
    sendAT(GF("+CSQ"));
  ...
  bool gprsConnect(const char* apn, const char* user, const char* pwd) {
    //Serial.println(F("##bool gprsConnect(const char* apn, const char* user, const char* pwd)"));
    gprsDisconnect();
  ...
  bool gprsDisconnect() {
    //Serial.println(F("##bool gprsDisconnect()"));
  ...
  bool isGprsConnected() {
    //Serial.println(F("##bool isGprsConnected()"));
    sendAT(GF("+CGATT?"));
  ...
  String getLocalIP() {
    //Serial.println(F("##String getLocalIP()"));
    sendAT(GF("+CIFSR;E0"));
    String res;
  ...
  bool setGsmBusy(bool busy = true) {
    //Serial.println(F("##bool setGsmBusy(bool busy = true)"));
    sendAT(GF("+GSMBUSY="), busy ? 1 : 0);
    return waitResponse() == 1;
  }

  bool callAnswer() {
    //Serial.println(F("##bool callAnswer()"));
    sendAT(GF("A"));
    return waitResponse() == 1;
  }
...
public:
  ...
  /*bool streamSkipUntil(char c) { //TODO: timeout
    Serial.println(F("##bool streamSkipUntil(char c)"));
    while (true) {
      while (!stream.available()) { TINY_GSM_YIELD(); }
      if (stream.read() == c)
        return true;
